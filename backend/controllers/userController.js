...
const registerUser = async (req, res) => {
  ...
  if (!isValidPassword(password)) {
    return res.status(400).json({
      message:
        'Password must contain at least one number and one special character and be 8 characters long',
    });
  }
...
const updatePassword = async (req, res) => {
  ...
  if (!isValidPassword(newPassword)) {
    return res.status(400).json({
      message:
        'New password must contain at least one number and one special character and be 8 characters long',
    });
  }
